...
class MainController < Trema::Controller
  ...
  def start(_argv)
    ...
    basic = ENV['HAVOX_BASIC'].eql?('true')
    logger.info 'Havox will ignore field conflicts in rules generated by Merlin'.blue if force
    logger.info 'Havox will automatically append policies for ARP and ICMP protocols'.blue if basic
    @rules = Havox::Policies.compile!(ENV['HAVOX_MERLIN_TOPOLOGY'], ENV['HAVOX_MERLIN_POLICY'], force, nil, basic)
    datapath_rules_info
  rescue => e
    handle_exception(e)
  end
