      ...
      module Actions
        ...
        def self.treat(actions_array, opts = {})
          ...
          actions_array.each do |obj|
            of_actions <<
              case obj[:action]
              when 'Output' then basic_action(:rfat_output, obj[:arg_a])
        ...
        def self.basic_action_from_set_field(obj)
          if obj[:arg_a].eql?('vlan')
            if obj[:arg_b].eql?('<none>')
              ...
              basic_action(:rfat_set_vlan_id, obj[:arg_b])  # Inferred (vandervecken).
            end
          else
        ...
        def self.output_or_enqueue(obj, change_to_output)
          if change_to_output
            basic_action(:rfat_output, obj[:arg_a])
          else
            basic_action(:rfat_enqueue, obj[:arg_a], obj[:arg_b])   # Inferred.
          end
        end
      end
    end
